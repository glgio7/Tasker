...
import { Container, CreateTask, InnerContainer, Notepad } from "./styles";
...
export default function Note() {
    ...
    const saveList = (value) => localStorage.setItem('savedTasks', JSON.stringify(value));
    ...
    return (
        <>
            ...
            <Container onload={() => window.scrollTo(0, 0)}>
                <h2>Notepad</h2>
                <span><span style={{ color: 'firebrick' }}>{remainingDays}</span> {remainingDays === 1 ? 'day' : 'days'} left for next month.</span>
                <InnerContainer>
                    ...
                    <Notepad>
                        ...
                        <iframe src="https://climaki.vercel.app" className={weather ? 'active' : ''} title="weather" crossOrigin="anonymous"></iframe>
                        <ul>
                            <CreateTask>
                                <input type="text" value={task} onChange={(e) => setTask(e.target.value)} onKeyDown={(e) => submitOnEnter(e)} />
                                ...
                                <button onClick={addTask}>Add</button></CreateTask>
                            {list.map(task => (
                                <Item key={task.id} checked={task.checked}>
                                    <p>{task.task}</p>
                                    <div className="edit"><p>{task.checked ? 'Done' : `Deadline: ${task.deadline ? task.deadline : 'Nope'}`}</p></div>
                                    <div className="edit">
                                        <button className="done" onClick={() => toggleDone(task.id, task.checked)}><MdOutlineDone /></button>
                                        <button><MdOutlineDelete onClick={() => removeTask(task.id)} /></button>
                                    </div>
                                </Item>
                            ))
                            }
                    ...
                    <div className="tip" onClick={toggleTip}>
                        ...
                        <p className={tip ? "" : "hidden-tip"}>Depending on whether your task is an indoor or outdoor task you should check the weather forecast.</p>
                    </div>
                    </Notepad>
                </InnerContainer>
            </Container>
